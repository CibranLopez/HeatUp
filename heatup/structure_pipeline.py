...
# ---------------------------------------------------------------------------
# When this module is run as a subprocess script (python .../structure_pipeline.py
# --_run relaxation ...) it executes as a plain file, not as part of an
# installed package.  Insert the project root (the directory that *contains*
# the heatup/ folder) into sys.path so "from heatup import ..." works without
# any pip install.
...
def _find_python() -> str:
    ...
    # Active conda / venv environments
    ...
    # Broad conda root scan — find any env that has mace installed
    conda_roots = [
    ...
    # Also honour CONDA_ROOT / MAMBA_ROOT_PREFIX if set
    ...
    # Nothing found — return sys.executable; the real error will appear in
    # the subprocess traceback.
...
def _cuda_env() -> dict[str, str]:
    ...
    * Forwards all live :mod:`heatup.config` overrides as ``HEATUP_CFG_*``
      ...
      notebook (e.g. ``cfg.MACE_MODEL = ...``).
    """
    env = os.environ.copy()
    env["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"

    # heatup is never pip-installed — always add its parent to PYTHONPATH.
    # This file lives at  <project_root>/heatup/structure_pipeline.py
    ...
    # Forward live config overrides to the subprocess.
